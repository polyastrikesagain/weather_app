...
class WeatherApi:
    ...
    def get_weather(self, current=False):
        ...
        response = requests.get(json_query).json()[fetch]
        ...
        for key in parameters:
            ...
            result.append((parameters[key], *response[key]))
        return result
    #make it so that every forecast is a bunch of values for each header: [weather code, temps, wind, uv_index]
    #and daily forecast for a week is basically a list of 7 lists